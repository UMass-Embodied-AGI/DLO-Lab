...
def main():
    ...
    if args.output_folder is not None:
        camera = scene.add_camera(
            res=(1024, 1024), pos=(-1.2, 1.5, 1.8), up=(0, 0, 1),
            lookat=(0., 0., 0.2), fov=args.fov, GUI = False
        )
    ...
    obj_elastic = scene.add_entity(
        material=gs.materials.MPM.Elastic(E=4e5, rho=200),
    ...
    obj_elastic.set_velocity(torch.tensor([0.8, 0.8, 0.0]))
    ...
    if args.output_folder is not None:
        ...
        mediapy.write_video(os.path.join(args.output_folder, f"coupling_four_rods_elastic{ray_traced}.mp4"), np.array(frames), fps=30)
        gs.logger.info(f"Video saved to {os.path.join(args.output_folder, f'coupling_four_rods_elastic{ray_traced}.mp4')}")
