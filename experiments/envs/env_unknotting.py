...
class Train_Env_Unknotting(Train_Env):
    ...
    def construct_cameras(self):
        ...
        cameras.append(self.scene.add_camera(
            res=(1200, 900), pos=(-0.5, 0, 0.55), up=(0, 0, 1),
            lookat=(0.7, -0.2, 0), fov=30, GUI=False
        ))
        if not self.raytracer:
            cameras.append(self.scene.add_camera(
                res=(1200, 900), pos=(1.8, -0.8, 0.5), up=(0, 0, 1),
                lookat=(0.7, -0.2, 0), fov=30, GUI=False
            ))
    ...
    @staticmethod
    def _unknotting_penalty(verts, interval=0.02):
        """Compute unknotting penalty for a batch of ropes.
        Args:
            verts: (n_envs, n_verts, 3)
            interval: float, distance between two vertices to start repulsion
        Returns:
            unknotting_penalty: (n_envs,)
        """
        # 1. Vectors and Midpoints
        edges = verts[:, 1:] - verts[:, :-1]  # (B, N-1, 3)
        midpoints = (verts[:, 1:] + verts[:, :-1]) / 2  # (B, N-1, 3)

        # 2. Broadcasting for O(N^2) pairs
        r_i = midpoints.unsqueeze(2)  # (B, N-1, 1, 3)
        r_j = midpoints.unsqueeze(1)  # (B, 1, N-1, 3)
        r_ij = r_i - r_j              # (B, N-1, N-1, 3)

        dr_i = edges.unsqueeze(2)     # (B, N-1, 1, 3)
        dr_j = edges.unsqueeze(1)     # (B, 1, N-1, 3)

        # 3. Scalar Triple Product
        cross = torch.cross(dr_i, dr_j, dim=-1)
        numerator = (r_ij * cross).sum(dim=-1)  # (B, N-1, N-1)

        # 4. Distance and Stability
        dist = torch.norm(r_ij, dim=-1)

        denominator = (dist ** 2 + 0.02 ** 2).pow(1.5)

        acn_matrix = torch.abs(numerator) / denominator

        # 5. Masking Neighbors (Critical for stability)
        n_edges = edges.shape[1]
        idx = torch.arange(n_edges, device=verts.device)
        diff = torch.abs(idx.unsqueeze(1) - idx.unsqueeze(0))
        mask = diff <= 1
        acn_matrix = acn_matrix.masked_fill(mask.unsqueeze(0), 0.0)

        # 6. Summation and Normalization
        sum_integral = acn_matrix.sum(dim=(1, 2))
        acn_value = sum_integral / (4 * torch.pi)

        # 7. Repulsion Term
        penetration = torch.relu(2 * interval - dist)
        penetration = penetration.masked_fill(mask.unsqueeze(0), 0.0)

        repulsion_term = (penetration ** 2).sum(dim=(1, 2))
        return acn_value + 100 * repulsion_term

    def reward(self):
        # [n_envs, n_verts, 3]
        verts_batch = self.rope.get_all_verts_tc()
        unknotting_penalty = self._unknotting_penalty(verts_batch)
        rewards = torch.exp(-unknotting_penalty).cpu().numpy()
    ...
    def loss_criterion(self, state):
        ...
        unknotting_loss = self._unknotting_penalty(verts_batch)
        return unknotting_loss  # (n_envs,)
    ...
    def step_diff_rl(self, env_mask, action):
        ...
        for j in range(n_steps_sub):
            ...
            # newly_collided = collided_ctrl & alive.cpu().numpy()
            # if newly_collided.any():
            #     alive[newly_collided] = False
