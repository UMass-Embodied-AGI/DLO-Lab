...
class SimOptions(Options):
    ...
    bptt_window : int, optional
        Truncated-BPTT window in *steps*. 0 (default) = full backprop-through-time (gradient flows
        through the entire trajectory). If > 0, during the backward pass the recurrent adjoint state
        is detached (zeroed) once it has propagated `bptt_window` steps past the most recent queried
        state, so each loss term's gradient only flows back that many steps. This caps the adjoint
        chain length and prevents the gradient blow-up that long, stiff/contact-rich rollouts
        produce, at the cost of ignoring longer-range sensitivities (a biased but stable estimator).
    ...
    bptt_window: int = 0  # truncated-BPTT window in steps; 0 = full BPTT
...
class LegacyCouplerOptions(BaseCouplerOptions):
    ...
    rod_mpm : bool, optional
        Whether to enable coupling between rod and MPM solvers. Defaults to True.
    """
...
class RigidOptions(Options):
    ...
    skip_backward : bool, optional
        ...
        rigid-body dynamics, reproducing the legacy behavior where the rigid backward was a no-op stub. Useful when the
        learning signal is taken downstream of the rigid solver (e.g. from a coupled deformable's vertex gradients) and
        the robot's own ``qpos``/``dofs_vel`` gradients are unused. Avoids NaN gradients originating from the
        forward-kinematics quaternion backward at near-degenerate configurations. Only has an effect when
        ``sim_options.requires_grad`` is True. Defaults to False.
    ...
    # differentiability
    skip_backward: StrictBool = False
...
class RODOptions(Options):
    ...
    grad_clip : float, optional
        If > 0, rescale the recurrent adjoint state (vertices/edges/internal-vertex grads) per
        batch to L2 norm <= grad_clip after each substep's backward, preserving its direction.
        This bounds the per-substep growth of the adjoint and removes the magnitude blow-up
        (NaN/inf) that long, contact-rich rollouts otherwise produce. NOTE: it bounds magnitude
        but does not by itself make a heavily contact-dominated adjoint *direction* reliable
        (those adjoints can be ill-conditioned / non-deterministic). 0 disables it (default).
    requires_grad_K : bool, optional
        If True, the rod stretching stiffness K is differentiable. Defaults to False.
    requires_grad_E : bool, optional
        If True, the rod bending stiffness E is differentiable. Defaults to False.
    requires_grad_G : bool, optional
    ...
    adjacent_gap: int = 2
    n_pbd_iters: PositiveInt = 10
    grad_clip: NonNegativeFloat = 0.0
    requires_grad_K: StrictBool = False
