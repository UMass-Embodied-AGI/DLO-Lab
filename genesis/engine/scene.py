...
class Scene(RBC):
    ...
    def __init__(
        ...
        self.rod_options = rod_options.model_copy_from(sim_options)
    ...
    def _validate_options(
        ...
        if not isinstance(rod_options, RODOptions):
            gs.raise_exception("`rod_options` should be an instance of `RODOptions`.")
    ...
    def add_entity(
        ...
        if isinstance(material, (gs.materials.Kinematic, gs.materials.Tool)):
            ...
            if surface.vis_mode not in ("recon", ):
                gs.raise_exception(
                    f"Unsupported `surface.vis_mode` for material {material}: '{surface.vis_mode}'. Expected one of: ['recon']."
                )
        ...
        entity = self._sim._add_entity(morph, material, surface, visualize_contact, visualize_twist, name)
    ...
    def add_stage(
        ...
        visualize_contact : bool
            ...
            Note that this will not be displayed in images rendered by camera using the `RayTracer` renderer.
        visualize_twist : bool
            Whether to visualize twist frames applied to this stage as arrows in the viewer and rendered images.
            Note that this will not be displayed in images rendered by camera using the `RayTracer` renderer.
        vis_mode : str | None, optional
        ...
        for entity_morph in entity_morphs:
            entities.append(self.add_entity(entity_morph, material, surface, visualize_contact, visualize_twist, vis_mode))
    ...
    def _step_grad(self):
        injected = self._sim.collect_output_grads()
    ...
    def _backward(self):
        ...
        # Backward pass through time. With truncated BPTT (sim.bptt_window > 0) the recurrent
        # adjoint is detached once it has propagated `window` steps past the most recent queried
        # state, so each loss term's gradient only flows back that many steps (caps the chain
        # length and prevents the long-horizon adjoint blow-up).
        window = self._sim._bptt_window
        since_inject = None  # steps since the last injection; None => currently detached
        while self._t > 0:
            injected = self._step_grad()
            if window > 0:
                if injected:
                    since_inject = 0
                elif since_inject is not None:
                    since_inject += 1
                if since_inject is not None and since_inject >= window:
                    self._sim.truncate_adjoint()
                    since_inject = None
