...
class Simulator(RBC):
    ...
    def truncate_adjoint(self):
        """Truncated BPTT: detach the recurrent adjoint in every solver that supports it."""
        for solver in self._active_solvers:
            if hasattr(solver, "truncate_adjoint"):
                solver.truncate_adjoint()
    ...
    def collect_output_grads(self):
        """
        Collect gradients from downstream queried states. Returns True if any state gradient was
        injected at the current step (used by truncated BPTT to reset its window).
        """
        ...
        for solver in self._active_solvers:
            if solver.collect_output_grads():
                injected = True
